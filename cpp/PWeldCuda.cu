...
// ----------------------------------------------------------------------------
// 🔒 Cross-platform safe type aliases and helpers
// ----------------------------------------------------------------------------
using CellKeyT = long long; // always 64-bit for large grids

// Safe inline hash for 3D grid cell coordinates
__host__ __device__ inline CellKeyT hash_cell_safe(int x, int y, int z, int3 grid_res) {
    return (CellKeyT)x
         + (CellKeyT)y * (CellKeyT)grid_res.x
         + (CellKeyT)z * (CellKeyT)grid_res.x * (CellKeyT)grid_res.y;
}
    ...
    cudaStream_t stream = 0)
{
    ...
    std::cout << "Grid resolution = ("
          ...
          // Ensure grid dimensions safe for 32-bit indexing
long long total_cells = 1LL * grid.resolution.x * grid.resolution.y * grid.resolution.z;
if (total_cells > INT_MAX) {
    std::cout << "⚠️  Large grid (" << total_cells
              << " cells) exceeds 32-bit range — safe mode enabled (64-bit keys)\n";
}
...
// --- Safe sort_by_key (64-bit + guarded) ---
{
    GpuTimer t("GPU sort_by_key (hash grid)", print_time, stream);

    const size_t key_size = d_keys.size();
    const size_t val_size = d_idx.size();

    if (key_size > 0 && key_size == val_size) {
        // sort safely using thrust::device policy
        thrust::sort_by_key(
            thrust::device,
        ...
        std::cout << "⚠️  Skipping sort: empty or mismatched vectors "
                  << "(keys=" << key_size << ", vals=" << val_size << ")\n";
    }
}
...
// ============================================================
{
    ...
    int iter = 0;
    {
        ...
        std::cout << "        (iters = " << iter << ")\n";
        // --- Print simplified vertices (clusters) count ---
{
    thrust::device_vector<int> d_is_centroid(nverts);
    thrust::transform(
        thrust::make_counting_iterator(0),
    ...
    int simplified_verts = thrust::reduce(
        d_is_centroid.begin(),
        d_is_centroid.end(),
        0,
        thrust::plus<int>());
    ...
    std::cout << "----------------------------------------------------------\n";
    std::cout << "Simplified vertices (clusters): " << simplified_verts
              << "  out of  " << nverts << " original vertices\n";
    std::cout << "----------------------------------------------------------\n";
}
...
if (nfaces > 0) {
    ...
    std::cout << "⚠️ No faces found — skipping mesh remap and face writing.\n";
    timeUmesh_s = 0.0;
    valid_faces = 0;
}
...
// ============================================================
{
    ...
    thrust::host_vector<double3> h_out_vertices(d_centroids.begin(), d_centroids.end());
    vertices.clear();
    vertices.reserve(h_out_vertices.size());
    for (auto& v : h_out_vertices)
        vertices.emplace_back(v.x, v.y, v.z);
    ...
    if (nfaces > 0) {
        // ---- Copy remapped valid faces ----
        thrust::host_vector<int3> h_out_faces(
            d_out_faces.begin(),
        ...
        triangles.clear();
        std::cout << "⚠️ No faces found — point cloud saved only.\n";
        std::cout << "✅ Copied back reduced cloud: "
                  << vertices.size() << " vertices.\n";
    }
